...
function hardcastle_cell_test(tbin_size, fc, redo, init_params)
    % PARAMETERS:
    ...
    % already exists
    % init_params - optional, hc_results.params_consol from a previous run
    % if want to speed up rerun
    
    if ~exist('redo', 'var')
        redo = false;
    end

    save_dir = [num2str(tbin_size*1000), 'ms_', num2str(fc), 'fold(test)'];
    ...
    if redo
       ...
       if exist('init_params', 'var')
           hc_results = glm_hardcastle_test(vmpvData, fc, [], init_params);
       else
           hc_results = glm_hardcastle_test(vmpvData, fc);
       end
    end

    % Run model forward selection with significance level of 0.05
    [selected_model, hc_results] = select_best_model_test(hc_results, 0.05);
