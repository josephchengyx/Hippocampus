...
function [selected_model, hc_results] = select_best_model_test(hc_results, p_sig)
    ...
    sig_tests = hc_results.testing_sigs;
    sig_tests = all(sig_tests < p_sig, 1);
    
    % Filter out models that did not pass significance test
    models = 1:7;
    models = models(sig_tests);

    % find the best single model
    singleModels = 5:7;
    singleModels = singleModels(sig_tests(singleModels));
    ...
    doubleModels = doubleModels(sig_tests(doubleModels));
    ...
    if p_llh_12 < p_sig % double model is sig. better
        if p_llh_23 < p_sig && sig_tests(top3)  % full model is sig. better
