function [lambda, cell_params] = simulate_cell_FR_custom(behav_state)
    % Inputs:
    ...
    cell_type = 'conjunctive pv';
    active_firing_rate = 5;
    background_firing_rate = 0.5;
    
    % place
    place_centers = [820]; % bin number
    place_widths = [6]; % in terms of number of bins
        
    % headdirection
    hd_centers = [NaN]; % bin number
    hd_widths = [NaN]; % in terms of number of bins
    
    % view
    view_centers = [5050, 4898]; % bin number
    view_widths = [3, 3]; % in terms of number of bins
    ...
    lambda = zeros(n,1);
    for i = 1:n
        if place_firing_rate_step(place_bin(i)) && view_firing_rate_step(view_bin(i))
            lambda(i) = active_firing_rate;
        else
            lambda(i) = background_firing_rate;
        end
    end
...
%%% Place %%%
function lambda = place_firing_rate_step(bin_num)
...
function lambda = place_firing_rate_gaussian(bin_num)
    % Parameters of the simulated place field
    global place_params;
    A = place_params{1}; % active firing rate
    B = place_params{2}; % background firing rate
...
%%% Head direction %%%
function lambda = hd_firing_rate_step(bin_num)
...
function lambda = hd_firing_rate_gaussian(bin_num)
    % Parameters of the simulated headdirection field
    global hd_params;
    A = hd_params{1}; % active firing rate
    B = hd_params{2}; % background firing rate
...
%%% View %%%
function lambda = view_firing_rate_step(bin_num)
...
function lambda = view_firing_rate_gaussian(bin_num)
    % Parameters of the simulated view field
    global view_params;
    A = view_params{1}; % active firing rate
    B = view_params{2}; % background firing rate
